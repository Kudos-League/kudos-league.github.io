...
import type { CreatePostDTO, PostDTO, CreateHandshakeDTO, UpdatePostDTO } from '@/shared/api/types';
...
export function useUpdatePost() {
    ...
    return useMutation<PostDTO, Error, UpdatePayload>({
        mutationFn: ({ id, data }) =>
            apiMutate<PostDTO, Partial<UpdatePostDTO>>(`/posts/${id}`, 'put', data, { as: 'form' }),
        onSuccess: (updated) => {
            ...
            qc.setQueriesData<{ data: PostDTO[]; nextCursor?: number; limit: number }>(
                ...
                (prev) =>
                    !prev
                        ? prev
                        : { ...prev, data: prev.data.map((p) => (p.id === updated.id ? { ...p, ...updated } : p)) }
            );
...
export function useLikePost() {
    ...
    return useMutation<void, Error, LikePayload, LikeCtx>({
        mutationFn: ({ id, like }) =>
            apiMutate<void, { like: boolean }>(`/posts/${id}/like`, 'post', { like }),
        onMutate: async ({ id, like }) => {
            ...
            qc.setQueriesData<{ data: PostDTO[] }>(infiniteFilter, (prev) =>
                !prev ? prev : { ...prev, data: prev.data.map((p) => (p.id === id ? ({ ...p, liked: like } as any) : p)) }
            );
...
export function useReportPost() {
    return useMutation<void, Error, ReportPayload>({
        mutationFn: ({ id, reason }) =>
            apiMutate<void, { reason: string }>(`/posts/${id}/report`, 'post', { reason }),
        onSuccess: () => {
...
export function useCreateHandshake() {
    ...
    return useMutation<{ data: any }, Error, CreateHandshakeDTO>({
        mutationFn: (payload) =>
            apiMutate<{ data: any }, CreateHandshakeDTO>('/handshakes', 'post', payload),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['posts'] });
            qc.invalidateQueries({ queryKey: ['posts', 'infinite'] });
...
export function useCreatePost() {
    ...
    return useMutation<PostDTO, Error, CreatePostDTO>({
        mutationFn: (payload) =>
            apiMutate<PostDTO, CreatePostDTO>('/posts', 'post', payload, { as: 'form' }),
        onSuccess: (created) => {
            qc.invalidateQueries({ queryKey: ['posts'] });
            qc.setQueryData<PostDTO[]>(['posts'], (prev) => (prev ? [created, ...prev] : [created]));
