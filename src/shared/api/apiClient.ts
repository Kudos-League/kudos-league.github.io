...
export async function apiGet<T>(url: string, opts?: { params?: any; headers?: any }) {
    try {
...
export async function apiMutate<T, B>(
    ...
    try {
        ...
        const res = await http.request<T>({ url, method, data, params: opts?.params, headers: { ...opts?.headers, ...headers } });
        return res.data;
    }
    catch (err) {
        throw extractApiErrors(err);
