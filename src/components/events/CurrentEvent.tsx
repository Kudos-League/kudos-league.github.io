            ...
            <div className='bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 transform transition-all'>
                <div className='flex items-center justify-between p-6 border-b border-gray-100'>
                    <div className='flex items-center gap-3'>
                        ...
                        <h3 className='text-lg font-semibold text-gray-900'>Location Required</h3>
                    </div>
                ...
                <div className='p-6'>
                    ...
                    <div className='space-y-4'>
                        <div className='flex items-start gap-3'>
                            <div className='bg-gray-100 rounded-full w-6 h-6 flex items-center justify-center text-sm font-medium text-gray-600 mt-0.5'>1</div>
                            <div className='flex-1'>
                                <div className='flex items-center gap-2 mb-1'>
                                    <User className='w-4 h-4 text-gray-500' />
                                    <span className='font-medium text-gray-900'>Go to your profile</span>
                                </div>
                                <p className='text-sm text-gray-600'>Click on your profile in the top right, then select &quot;Profile&quot;.</p>
                            </div>
                        </div>
                        <div className='flex items-start gap-3'>
                            <div className='bg-gray-100 rounded-full w-6 h-6 flex items-center justify-center text-sm font-medium text-gray-600 mt-0.5'>2</div>
                            <div className='flex-1'>
                                <div className='flex items-center gap-2 mb-1'>
                                    <Edit3 className='w-4 h-4 text-gray-500' />
                                    <span className='font-medium text-gray-900'>Edit your profile</span>
                                </div>
                                <p className='text-sm text-gray-600'>Click the &quot;Edit&quot; button to modify your profile settings.</p>
                            </div>
                        </div>
                        <div className='flex items-start gap-3'>
                            <div className='bg-gray-100 rounded-full w-6 h-6 flex items-center justify-center text-sm font-medium text-gray-600 mt-0.5'>3</div>
                            <div className='flex-1'>
                                <div className='flex items-center gap-2 mb-1'>
                                    <MapPin className='w-4 h-4 text-gray-500' />
                                    <span className='font-medium text-gray-900'>Add your location</span>
                                </div>
                                <p className='text-sm text-gray-600'>Enter your location and click &quot;Save Changes&quot;.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='flex gap-3 p-6 bg-gray-50 rounded-b-xl'>
                    <Button onClick={onClose} className='flex-1 px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors font-medium'>
                        Got it
                    </Button>
                </div>
            </div>
...
export default function CurrentEvent() {
    ...
    const [timeFilter, setTimeFilter] = useState<'all' | 'today' | 'next7d' | 'nextmonth'>('all');
    ...
    const { data: events = [], isLoading, isError } = useEvents(serverFilters, {
        enabled,
        select: selectByTime
    });
    ...
    return (
        <div className='my-6 px-4'>
            <h2 className='text-xl font-bold mb-2 text-center'>Active Events</h2>

            <div className='flex items-center justify-between mb-4'>
                <Button
                    onClick={() => setLocationFilter((prev) => !prev)}
                    className={`px-4 py-1 rounded transition-colors ${
                        locationFilter ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-300 hover:bg-gray-400'
                    }`}
                >
            ...
            {isLoading ? (
                ...
                <p className='text-center text-red-600'>Failed to fetch events.</p>
            ) : events.length > 0 ? (
                <div className='flex items-center justify-center gap-4 list-none'>
                    <Button
                        onClick={() => setCurrentIndex((i) => (i - 1 + events.length) % events.length)}
                        variant='icon'
                        shape='circle'
                        className='w-8 h-8'
                    >
                    ...
                    <Button
                        onClick={() => setCurrentIndex((i) => (i + 1) % events.length)}
                        variant='icon'
                        shape='circle'
                        className='w-8 h-8'
                    >
                ...
                <p className='text-center text-gray-600 italic mt-4'>No events match your filter.</p>
            )}
