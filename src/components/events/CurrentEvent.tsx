...
export default function CurrentEvent() {
  ...
  return (
    <div className="my-6 px-4">
      <h2 className="text-xl font-bold mb-2 text-center">Currently Ongoing Events</h2>

      <div className="flex items-center justify-between mb-4">
        <button
          onClick={() => setLocationFilter((prev) => !prev)}
          className={`px-4 py-1 rounded ${
            locationFilter ? "bg-green-600 text-white" : "bg-gray-300"
          }`}
        >
        ...
        <select
          value={timeFilter}
          onChange={(e) => setTimeFilter(e.target.value as "24h" | "7d" | "all")}
      ...
      {events.length > 0 ? (
        <div className="flex items-center justify-center gap-4">
          <button onClick={() => setCurrentIndex((i) => (i - 1 + events.length) % events.length)}>
            ◀
          </button>
          ...
          >
            ...
            <p className="text-gray-600 mb-2">{currentEvent.description}</p>
          ...
          <button onClick={() => setCurrentIndex((i) => (i + 1) % events.length)}>▶</button>
        </div>
      ) : (
