        ...
        (events: any[], timeFilter: string) => {
            ...
            return events.filter((event) => {
                ...
                case 'today':
                    ...
                    return (
                        (eventStart.isBefore(endOfToday) ||
                                eventStart.isSame(endOfToday)) &&
                            (eventEnd.isAfter(startOfToday) ||
                                eventEnd.isSame(startOfToday))
                    );

                case 'next7d': {
                    const endOfNext7Days = dayjs(now)
                        .add(7, 'day')
                        .endOf('day');
                        // Event is active in next 7 days if it starts before/on end of next 7 days AND ends after/on start of today
                    return (
                        (eventStart.isBefore(endOfNext7Days) ||
                                eventStart.isSame(endOfNext7Days)) &&
                            (eventEnd.isAfter(startOfToday) ||
                                eventEnd.isSame(startOfToday))
                    );
                }
