...
type SuspectedLinkedUser = { id: number; username?: string | null; email?: string | null; admin?: boolean; ips: string[]; lastSeen?: string | Date | null };
type HandshakeView = { id: number; postID?: number; senderID: number; receiverID: number; type?: string; status?: string; createdAt?: string | Date };
...
export default function AdminReportModal({ open, onClose, userID }: { open: boolean; onClose: () => void; userID: number | null }) {
    ...
    const [suspectsNext, setSuspectsNext] = useState<string | number | undefined>(undefined);
    ...
    const [handshakesNext, setHandshakesNext] = useState<string | number | undefined>(undefined);
    ...
    useEffect(() => {
        if (!open) return;
        setLoginAudits([]); setLoginNext(undefined); setLoginError(null);
        setSuspects([]); setSuspectsNext(undefined); setSuspectsError(null);
        setHandshakes([]); setHandshakesNext(undefined); setHandshakesError(null);
        if (userID) {
    ...
    async function loadMoreLogin(id: number) {
        ...
        try {
            const q = loginNext ? `?limit=${limit}&cursor=${encodeURIComponent(String(loginNext))}` : `?limit=${limit}`;
            const res = await apiGet<Paged<LoginAudit>>(`/admin/users/${id}/login-audits${q}`);
    ...
    async function loadMoreSuspects(id: number) {
        setSuspectsLoading(true); setSuspectsError(null);
        try {
            const q = suspectsNext ? `?limit=${limit}&cursor=${encodeURIComponent(String(suspectsNext))}` : `?limit=${limit}`;
            const res = await apiGet<Paged<SuspectedLinkedUser>>(`/admin/users/${id}/suspected-linked${q}`);
    ...
    async function loadMoreHandshakes(id: number) {
        setHandshakesLoading(true); setHandshakesError(null);
        try {
            const q = handshakesNext ? `?limit=${limit}&cursor=${encodeURIComponent(String(handshakesNext))}` : `?limit=${limit}`;
            const res = await apiGet<Paged<HandshakeView>>(`/admin/users/${id}/shared-handshakes${q}`);
    ...
    return (
        <Modal open={open} onClose={onClose} title={`Admin report for ${userID ?? ''}`}>
            <div className='max-w-xl'>
                ...
                {activeTab === 'login' && (
                    <div>
                        {loginLoading && loginAudits.length === 0 && <div className='flex items-center gap-2'><div className='animate-spin rounded-full h-4 w-4 border-b-2 border-current' /> Loading...</div>}
                        {loginError && <div className='text-red-600'>{loginError}</div>}
                        {loginAudits.length ? (
                            <ul className='text-sm list-disc pl-5'>
                                {loginAudits.map((a, i) => (
                                    <li key={i}>{a.ipAddress} — {a.region ?? a.country ?? 'Unknown'} — {new Date(a.createdAt).toLocaleString()}</li>
                                ))}
                            </ul>
                        ) : (!loginLoading && <div className='text-sm text-gray-600'>No login audits</div>)}
                        <div className='mt-2'>
                            {loginNext ? <Button variant='ghost' shape='default' className='text-sm' onClick={() => userID && loadMoreLogin(userID)} disabled={loginLoading}>{loginLoading ? 'Loading...' : 'Load more'}</Button> : null}
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'suspects' && (
                    <div>
                        {suspectsLoading && suspects.length === 0 && <div className='flex items-center gap-2'><div className='animate-spin rounded-full h-4 w-4 border-b-2 border-current' /> Loading...</div>}
                        {suspectsError && <div className='text-red-600'>{suspectsError}</div>}
                        {suspects.length ? (
                            <ul className='text-sm list-disc pl-5'>
                                {suspects.map((s) => (
                                    <li key={s.id}>{s.username ?? s.email ?? `#${s.id}`} — IPs: {s.ips.join(', ')} — last seen: {s.lastSeen ? new Date(s.lastSeen).toLocaleString() : 'unknown'}</li>
                                ))}
                            </ul>
                        ) : (!suspectsLoading && <div className='text-sm text-gray-600'>No linked users</div>)}
                        <div className='mt-2'>
                            {suspectsNext ? <Button variant='ghost' shape='default' className='text-sm' onClick={() => userID && loadMoreSuspects(userID)} disabled={suspectsLoading}>{suspectsLoading ? 'Loading...' : 'Load more'}</Button> : null}
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'handshakes' && (
                    <div>
                        {handshakesLoading && handshakes.length === 0 && <div className='flex items-center gap-2'><div className='animate-spin rounded-full h-4 w-4 border-b-2 border-current' /> Loading...</div>}
                        {handshakesError && <div className='text-red-600'>{handshakesError}</div>}
                        {handshakes.length ? (
                            <ul className='text-sm list-disc pl-5'>
                                {handshakes.map((h) => (
                                    <li key={h.id}>Handshake #{h.id} — sender: {h.senderID} receiver: {h.receiverID} — status: {h.status ?? 'unknown'}</li>
                                ))}
                            </ul>
