...
function LineChart({ data }: { data: MAUPoint[] }) {
    ...
    const months = data.map((d) =>
        new Date(d.month).toLocaleDateString(undefined, { month: 'short' })
    );
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className='w-full h-auto'>
            ...
            {months.map((m, i) => {
                const x =
                    32 + (i * (width - 64)) / Math.max(1, months.length - 1);
                return (
                    <text
                        key={i}
                        x={x}
            ...
            {data.map((d, i) => {
                ...
                return (
                    <circle key={i} cx={x} cy={y} r={3} fill={palette.line} />
                );
            })}
        </svg>
    );
...
export default function AdminAnalytics() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await apiGet<{ month: string; count: number }[]>(
                    '/admin/analytics/mau'
                );
        ...
        <div className='space-y-4'>
            ...
            <div className='grid grid-cols-3 sm:grid-cols-6 gap-2 text-sm'>
                {mau.map((d) => (
                    <div
                        key={d.month}
                        className='flex justify-between border rounded px-2 py-1 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 text-gray-800 dark:text-gray-100'
                    >
