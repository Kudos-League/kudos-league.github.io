...
const Profile: React.FC<Props> = ({ user, setUser, hideBackButton = false, hideWrapper = false }) => {
    ...
    const content = (
        <div className={hideWrapper ? 'space-y-6' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-lg shadow-lg px-4 sm:px-6 lg:px-8 py-8 space-y-6'}>
            <ProfileHeader
                user={user}
                userSettings={user.settings}
                isSelf={isSelf}
