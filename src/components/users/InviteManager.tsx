...
export default function InviteManager() {
    const form = useForm<InviteFormValues>({ mode: 'onBlur', defaultValues: { email: '' } });
    const [invites, setInvites] = useState<UserInviteDTO[]>([]);
    const [loading, setLoading] = useState(true);
    const [loadingMore, setLoadingMore] = useState(false);
    const [creating, setCreating] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [success, setSuccess] = useState<{ inviteUrl?: string | null; token: string } | null>(null);
    ...
    const loadInvites = useCallback(async (cursor?: number, append = false) => {
        ...
        try {
            ...
            const data = await apiGet<UserInviteListResponse>('/users/invites', {
                params
            });

            const invitesList = Array.isArray(data?.invites) ? data.invites : [];
            const next = typeof data?.nextCursor === 'number' ? data.nextCursor : null;
            setInvites((prev) => (append ? prev.concat(invitesList) : invitesList));
            setNextCursor(next);
        }
        catch (err: any) {
            const message = err?.response?.data?.message || err?.message || 'Failed to load invites.';
            setError(message);
            if (!append) {
                setInvites([]);
                setNextCursor(null);
    ...
    const handleRevokeInvite = async (inviteID: number) => {
        try {
            ...
            const message = err?.response?.data?.message || err?.message || 'Failed to revoke invite.';
            setError(message);
        }
        finally {
            setRevoking(null);
    ...
    const handleCreateInvite = async (values: InviteFormValues) => {
        try {
            ...
            const payload = values.email && values.email.trim().length > 0
    ...
    return (
        <div className='rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm p-5 space-y-4'>
            <div>
                <h3 className='text-lg font-semibold text-slate-800 dark:text-zinc-100'>Manage invites</h3>
                <p className='text-sm text-slate-500 dark:text-zinc-400'>Generate invite links for new members and track their status.</p>
            </div>
            ...
            {success && (
                <Alert tone='success'>
                    <div className='space-y-1'>
                        ...
                        {success.inviteUrl ? (
                            <p className='break-all text-xs'>{success.inviteUrl}</p>
                        ) : (
                            <p className='break-all text-xs'>Token: {success.token}</p>
                        )}
                    </div>
                </Alert>
            )}
            ...
            <Form methods={form} onSubmit={handleCreateInvite} className='flex flex-col gap-3 sm:flex-row sm:items-end'>
            ...
            <div className='space-y-2'>
                <h4 className='text-sm font-semibold text-slate-700 dark:text-zinc-200 uppercase tracking-wide'>Your invites</h4>
                {loading ? (
                    <p className='text-sm text-slate-500 dark:text-zinc-400'>Loading…</p>
                ) : invites.length === 0 ? (
                    <p className='text-sm text-slate-500 dark:text-zinc-400'>No invites yet. Generate one to get started.</p>
                ) : (
                    <div className='space-y-2'>
                        {invites.map((invite) => {
                            const usedBy = invite.usedBy ?? null;
                            const isDisabled = !!(invite.revoked || invite.usedAt || invite.usedByUserID);

                            return (
                                <div key={invite.id} className='flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-lg border border-slate-200 dark:border-zinc-700 px-3 py-2 text-sm text-slate-600 dark:text-zinc-300'>
                                    <div className='flex-1 space-y-1'>
                                        {usedBy ? (
                                            <>
                                                <p className='text-xs uppercase text-slate-500 dark:text-zinc-400 tracking-wide'>Invite redeemed</p>
                                                <UserCard
                                                    user={usedBy}
                                                    triggerVariant='avatar-name'
                                                    subtitle={invite.usedAt ? `Joined ${formatDateTime(invite.usedAt)}` : 'Member joined'}
                                                    disableTooltip
                                                    interactive={false}
                                                />
                                            </>
                                            ...
                                            <>
                                                <p className='font-medium text-slate-700 dark:text-zinc-100'>
                                                    {invite.targetEmail || 'Shareable link'}
                                                </p>
                                    ...
                                    <div className='flex items-center gap-3 text-xs font-medium text-slate-500 dark:text-zinc-400'>
                                        <span>{renderInviteStatus(invite)}</span>
                                        <button
                                            type='button'
                                            onClick={() => handleRevokeInvite(invite.id)}
                                            disabled={isDisabled || isRevoking === invite.id}
