...
const PayloadModal = ({ isOpen, payload, title, onClose }: {
    isOpen: boolean
    payload: any
    title: string
    onClose: () => void
...
const LogRow = React.memo(({ log, onAddToRecording, onAddFromHere, onSelectRangeStart, isRangeStart, isPinned, onPin }: {
    log: LogEntry
    ...
    const [payloadModal, setPayloadModal] = useState<{ isOpen: boolean; payload: any; title: string }>({
        isOpen: false,
        payload: null,
        title: '',
    });
    ...
    const getTruncatedPayload = (payload: any): string => {
        ...
        if (json.length > PAYLOAD_TRUNCATE_LENGTH) {
            return json.substring(0, PAYLOAD_TRUNCATE_LENGTH) + '\n... (truncated)';
        }
        return json;
    };
    ...
    return (
        <div className={`border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-slate-900/50 transition-colors ${isPinned ? 'bg-yellow-50 dark:bg-yellow-900/20 border-l-2 border-l-yellow-500' : ''}`}>
            <div className='flex items-center gap-2 px-3 py-2 group'>
                ...
                >
                    <div className='flex items-center gap-2 text-xs'>
                        ...
                        <span className={`px-2 py-0.5 rounded font-semibold text-xs flex-shrink-0 ${getTypeColor()}`}>
                            {getTypeLabel()}
                        </span>
                                ...
                                {(log as any).requestBody && (
                                    <div>
                                        <div className='flex items-center justify-between mb-1'>
                                            ...
                                            {JSON.stringify((log as any).requestBody, null, 2).length > PAYLOAD_TRUNCATE_LENGTH && (
                                                <button
                                                    onClick={() => openPayloadModal((log as any).requestBody, 'Request Body')}
                                        ...
                                        <pre className='bg-gray-200 dark:bg-slate-800 p-2 rounded overflow-auto max-h-48 text-xs'>
                                            {getTruncatedPayload((log as any).requestBody)}
                                        </pre>
                                    </div>
                                )}
                                {(log as any).responseBody && (
                                    <div>
                                        <div className='flex items-center justify-between mb-1'>
                                            ...
                                            {JSON.stringify((log as any).responseBody, null, 2).length > PAYLOAD_TRUNCATE_LENGTH && (
                                                <button
                                                    onClick={() => openPayloadModal((log as any).responseBody, 'Response Body')}
                                                    className='text-xs text-purple-600 dark:text-purple-400 hover:underline font-medium'
                                                >
                                                    View Full
                                                </button>
                                        ...
                                        <pre className='bg-gray-200 dark:bg-slate-800 p-2 rounded overflow-auto max-h-48 text-xs'>
                                            {getTruncatedPayload((log as any).responseBody)}
                                        </pre>
                                    </div>
                                )}
                            </div>
            ...
            <PayloadModal
                ...
                onClose={() => setPayloadModal({ isOpen: false, payload: null, title: '' })}
            />
        </div>
    );
});
...
export default function LogsSection({ selectedLogs, onSelectedLogsChange }: LogsSectionProps) {
    ...
    useEffect(() => {
        localStorage.setItem('kudos-logs-pinned-ids', JSON.stringify(Array.from(pinnedLogIds)));
    }, [pinnedLogIds]);
    ...
    const handleStatisticClick = (filterType: 'all' | 'console' | 'network' | 'websocket' | 'react-query' | 'errors' | 'warnings') => {
    ...
    const handleSelectRangeStart = (logId: string) => {
        ...
        const startIndex = filteredLogs.findIndex((l) => l.id === rangeStartLogId);
                ...
                <div className='bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded p-2'>
                    ...
                    <div className='space-y-1 max-h-24 overflow-y-auto'>
                        {Array.from(pinnedLogIds).map((pinnedId) => {
                            const pinnedLog = logs.find((l) => l.id === pinnedId);
                            if (!pinnedLog) return null;
                            const isInView = filteredLogs.some((l) => l.id === pinnedId);
                            return (
                                <div
                                    key={pinnedId}
                                ...
                                >
                                    {editingPinId === pinnedId ? (
                                        <input
                                            ...
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') handleSaveRename(pinnedId);
                                                if (e.key === 'Escape') handleCancelRename();
                                            }}
                                            onBlur={() => handleSaveRename(pinnedId)}
                                        ...
                                        <span
                                            onClick={() => handleStartRename(pinnedId, pinnedLogNames.get(pinnedId) || pinnedLog.message)}
                                            className='flex-1 truncate font-mono text-yellow-900 dark:text-yellow-100 cursor-pointer hover:underline'
                                            title='Click to rename'
                                        >
                                            {pinnedLogNames.get(pinnedId) || pinnedLog.message}
                                        </span>
                                    )}
                                    <button
                                        onClick={() => handleJumpToPin(pinnedId)}
                                        disabled={!isInView}
                                        ...
                                        title={isInView ? 'Jump to pinned log' : 'Out of view (filters hiding this log)'}
                                    >
                                        →
                                    </button>
                                    <button
