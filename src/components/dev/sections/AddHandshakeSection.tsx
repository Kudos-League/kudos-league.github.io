...
export default function AddHandshakeSection() {
    ...
    const [createdHandshakes, setCreatedHandshakes] = useState<CreatedHandshake[]>([]);
    const [updatingHandshake, setUpdatingHandshake] = useState<number | null>(null);
    ...
    useEffect(() => {
        sessionStorage.setItem(COMPLETED_POSTS_KEY, JSON.stringify(Array.from(completedPostIds)));
    }, [completedPostIds]);
    ...
    const createPost = async (postType: 'gift' | 'request') => {
        ...
        try {
            ...
            const randomBody = RANDOM_BODIES[Math.floor(Math.random() * RANDOM_BODIES.length)];
    ...
    const searchPosts = async () => {
        ...
        try {
            const results = await apiGet<PostDTO[]>('/posts/search', { params: { q: postSearchQuery, limit: 5 } });
    ...
    const createHandshakeForUser = async (user: UserDTO) => {
        ...
        try {
            ...
            if (freshPost) {
                const existingHandshake = (freshPost.handshakes || []).find(h => h.senderID === user.id);
                if (existingHandshake) {
                    alert(`User #${user.id} already has a ${existingHandshake.status} handshake on this post`);
            ...
            const handshake = await apiMutate<HandshakeDTO, any>('/dev/handshakes/create', 'post', {
            ...
            if (autoTestFlow) {
                ...
                await apiMutate(`/dev/handshakes/${handshake.id}/update`, 'post', {
                    status: 'accepted',
            ...
            setCreatedHandshakes(prev => [...prev, {
                ...handshake,
    ...
    const updateHandshakeStatus = async (handshakeId: number, newStatus: 'accepted' | 'completed', asUserId?: number) => {
        setUpdatingHandshake(handshakeId);
        try {
            // Use dev endpoint for impersonation
            if (asUserId) {
                await apiMutate(`/dev/handshakes/${handshakeId}/update`, 'post', {
                    status: newStatus,
                    userId: asUserId
                });
            }
            else {
                await apiMutate(`/handshakes/${handshakeId}`, 'patch', { status: newStatus });
            }

            // Update local state
            setCreatedHandshakes(prev => prev.map(hs =>
    ...
    const deleteHandshake = async (handshakeId: number) => {
        ...
        try {
            ...
            setCreatedHandshakes(prev => prev.filter(hs => hs.id !== handshakeId));
    ...
    const PostItem = ({ post, isSelected, onSelect }: { post: PostDTO; isSelected: boolean; onSelect: () => void }) => {
        ...
        return (
            ...
            >
                ...
                <div className="font-medium text-gray-800 dark:text-gray-200 truncate pr-12">{post.title}</div>
    ...
    const getHandshakeForUser = (userId: number): HandshakeDTO | undefined => {
        return (selectedPost?.handshakes || []).find(h => h.senderID === userId);
    };
    ...
    const UserItem = ({ user, onAdd, disabled, disabledReason, isCreating }: {
        user: UserDTO;
        onAdd: () => void;
        disabled?: boolean;
        ...
        <div className={`flex items-center gap-2 p-2 rounded-lg text-xs transition-colors ${
            disabled
            ...
            <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 dark:text-gray-200 truncate">
                    ...
                    {disabledReason && <span className="ml-2 text-red-500 text-[10px]">({disabledReason})</span>}
                </div>
            ...
            <button
                ...
                title={disabled ? disabledReason : 'Add handshake for this user'}
            >
                {isCreating ? '...' : '+'}
            </button>
        </div>
    ...
    const HandshakeCard = ({ hs }: { hs: CreatedHandshake }) => {
        ...
        const StatusBadge = () => (
            <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${
                hs.status === 'new' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' :
                    hs.status === 'accepted' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' :
                        hs.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
                            hs.status === 'cancelled' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300' :
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 text-xs overflow-hidden">
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-2">
                        <span className="font-semibold text-gray-800 dark:text-gray-200">HS #{hs.id}</span>
                    ...
                    {hs.status !== 'completed' && hs.status !== 'cancelled' && (
                        <div className="flex gap-1">
                            <button
                                onClick={() => updateHandshakeStatus(hs.id, 'cancelled' as any, handshakeSenderId)}
                                disabled={isUpdating}
                    ...
                    <UserPanel
                        ...
                        actions={
                            <>
                                {canUserAAccept && (
                                    <button
                                        onClick={() => updateHandshakeStatus(hs.id, 'accepted', postCreatorId)}
                                        disabled={isUpdating}
                                ...
                                {!isGift && canReceiverComplete && (
                                    <button
                                        onClick={() => updateHandshakeStatus(hs.id, 'completed', postCreatorId)}
                                        disabled={isUpdating}
                                    ...
                                    >
                                        {isUpdating ? '...' : 'Complete + Kudos'}
                                    </button>
                                )}
                                {hs.status === 'new' && (
                                    <span className="text-[10px] text-gray-400 italic">waiting to accept...</span>
                                )}
                                {hs.status === 'accepted' && isGift && (
                                    <span className="text-[10px] text-gray-400 italic">waiting for receiver...</span>
                                )}
                                {hs.status === 'completed' && (
                    ...
                    <UserPanel
                        ...
                        actions={
                            <>
                                {isGift && canReceiverComplete && (
                                    <button
                                        onClick={() => updateHandshakeStatus(hs.id, 'completed', handshakeSenderId)}
                                        disabled={isUpdating}
                                        className="px-2 py-1 bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white rounded text-[10px]"
                                ...
                                {hs.status === 'new' && (
                                    <span className="text-[10px] text-gray-400 italic">waiting for acceptance...</span>
                                )}
                                {hs.status === 'accepted' && !isGift && (
                                    <span className="text-[10px] text-gray-400 italic">waiting for receiver...</span>
                                )}
                ...
                <div className="px-3 py-1.5 bg-gray-50 dark:bg-gray-750 border-t border-gray-200 dark:border-gray-700 text-[10px] text-gray-500 dark:text-gray-400">
                    {isGift ? (
                        <>User #{postCreatorId} offers → User #{handshakeSenderId} requests → Accept → User #{handshakeSenderId} receives & sends kudos</>
                    ) : (
                        <>User #{postCreatorId} requests → User #{handshakeSenderId} offers help → Accept → User #{postCreatorId} receives & sends kudos</>
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Add Handshakes to Post</h3>
            ...
            <div className="space-y-2">
                ...
                {/* Recent Posts */}
                <div className="space-y-1">
                    <div className="text-[10px] text-gray-500 uppercase">Recent Posts</div>
                    {loadingPosts ? (
                ...
                {selectedPost && (
                    <div className="p-2 bg-purple-50 dark:bg-purple-900/30 rounded-lg border-2 border-purple-400 dark:border-purple-600 shadow-md">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                onClick={() => { setSelectedPost(null); setCreatedHandshakes([]); }}
                        ...
                        <div className="text-xs text-gray-700 dark:text-gray-300 mt-1">
                            #{selectedPost.id} - {selectedPost.title} ({selectedPost.type})
                        </div>
                        ...
                        <div className="mt-2 pt-2 border-t border-purple-200 dark:border-purple-700 flex items-center justify-center gap-2 text-purple-600 dark:text-purple-400">
                            <span className="text-xs font-medium">Continue below to add handshakes</span>
                            <span className="animate-bounce text-lg">↓</span>
                        </div>

                        {/* Existing Handshakes on this post */}
                        {selectedPost.handshakes && selectedPost.handshakes.length > 0 && (
                            <div className="mt-2 pt-2 border-t border-purple-200 dark:border-purple-700">
                                <div className="text-[10px] text-purple-600 dark:text-purple-400 font-medium mb-1">
                                    Existing Handshakes ({selectedPost.handshakes.length}):
                                </div>
                                <div className="space-y-1">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                onChange={(e) => setAutoTestFlow(e.target.checked)}
                    ...
                    {autoTestFlow ? (
                        <div className="p-2 bg-purple-50 dark:bg-purple-900/30 rounded-lg text-xs text-purple-700 dark:text-purple-300">
                            Will run full flow: new → accept → complete (with kudos)
                        </div>
                    ) : (
                        <div className="flex gap-2">
                            <button
                    ...
                    {userSearchResults.length > 0 && (
                        <div className="space-y-1">
                            ...
                            {userSearchResults.map(user => {
                                const disabledReason = getUserDisabledReason(user.id);
                                return (
                                    <UserItem
                                        key={user.id}
                                        user={user}
                            ...
                            recentUsers.map(user => {
                                ...
                                return (
                                    <UserItem
                                        ...
                                        disabledReason={disabledReason}
                                        isCreating={creatingForUser === user.id}
                                        onAdd={() => createHandshakeForUser(user)}
                                    />
                                );
                            })
                        ) : (
