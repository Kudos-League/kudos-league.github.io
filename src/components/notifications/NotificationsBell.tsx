                ...
                {open && (
                    <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-auto bg-white shadow rounded z-50">
                        <div className="flex justify-between items-center p-2 border-b">
                            ...
                            <button className="text-sm underline" onClick={markAllRead}>Mark all read</button>
                        </div>
                        <ul>
                            {!state.loaded ? (
                                <li className="p-3 text-sm text-gray-500">Loading…</li>
                            ) : state.items.length === 0 ? (
                                <li className="p-3 text-sm text-gray-500">No notifications yet</li>
                            ) : (
                                state.items.map((n, i) => (
                                    <li key={i} className="p-3 hover:bg-gray-50 cursor-pointer" onClick={() => go(n)}>
                                        {n.type === 'direct-message' ? (
                                            <div>
                                                <div className="font-medium">New DM</div>
                                                <div className="text-sm text-gray-600 truncate">{n.message?.content}</div>
                                            </div>
                                        ) : (
                                            <div>
                                                <div className="font-medium">Reply to your post</div>
                                                <div className="text-sm text-gray-600 truncate">{n.message?.content}</div>
                                            </div>
                                        )}
                                    </li>
