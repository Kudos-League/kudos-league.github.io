...
const ProfileHeader: React.FC<Props> = ({
  ...
  return (
    <div className="text-center px-6 pt-6 pb-8 bg-white shadow rounded">
      ...
      <p className="text-gray-600 text-sm">{user.kudos.toLocaleString()} Kudos</p>
      ...
      <div className="flex justify-center gap-4 mt-6">
        {isLoggedIn && !isSelf && (
          <button onClick={onStartDM} className="px-4 py-2 rounded bg-blue-100 hover:bg-blue-200">
            💬 Message
          </button>
        )}
        {isSelf && (
          <button onClick={onEditProfile} className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300">
            ⚙️ Edit
          </button>
        )}
      </div>
      ...
      {user.location?.regionID && (
        <div className="mt-6">
          <h3 className="text-sm font-semibold mb-1 text-gray-600">Location</h3>
          <MapDisplay
