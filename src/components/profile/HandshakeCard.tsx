...
import { getUserDetails, updateHandshake, createRewardOffer } from "shared/api/actions";
    ...
    <div className="border p-4 rounded shadow bg-white space-y-3">
      ...
      {status === "accepted" &&
        ((handshake.post.type === "request" && isSender) ||
          (handshake.post.type === "gift" && handshake.senderID.toString() === userId)) && (
          <div className="space-y-2">
            <label className="block text-sm font-medium">Assign Kudos</label>
            <input
              type="number"
              value={kudosValue}
