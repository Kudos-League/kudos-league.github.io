...
export default function Activity({user, hideWrapper = false}: Props) {
    const { isLoggedIn, user: currentUser } = useAuth();
    const navigate = useNavigate();
    const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;

    // Check if viewing own activity (kudos history only available for own profile)
    const isOwnActivity = currentUser?.id === user?.id;
    ...
    const renderFilteredContent = () => {
        // For other users, don't include handshakes in empty state check
        const showEmptyState = isOwnActivity
            ? sortedPosts.length === 0 && sortedEvents.length === 0 && sortedHandshakes.length === 0
            : sortedPosts.length === 0 && sortedEvents.length === 0;
        ...
        return (
            <div className='space-y-8'>
                ...
                {/* Handshakes Section - only for own activity */}
                {isOwnActivity && sortedHandshakes.length > 0 && (
                    <div>
                        <h3 className='text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100'>
    ...
    if (loading) {
        return <Spinner text='Loading activity...' variant={hideWrapper ? 'inline' : 'fullscreen'} />;
    }
    ...
    // When embedded (hideWrapper=true), render simplified version without outer wrapper
    if (hideWrapper) {
        return (
            <div className='space-y-4'>
                {/* Filter Buttons */}
                ...
                {/* Sub-filters - visible only on large screens */}
                {renderSubFilters()}
                                ...
                                {filter === 'all' && (
                                    ...
                                    >
                                        {isOwnActivity
                                            ...
                                            : 'Browse recent posts and events'}
                                    </p>
                                )}
                            </>
                        )}
