...
export function TextInput({ rounded = 'both', className = '', ...props }: InputProps) {
    return <input {...props} className={`${baseInput} ${radius[rounded]} ${className}`} />;
}
...
export function Alert({
  ...
  children?: React.ReactNode;
    ...
    const tones = {
        error:
      'bg-red-50 border border-red-200 rounded-md p-3 text-sm text-red-700 dark:bg-red-500/10 dark:border-red-500/30 dark:text-red-200',
        success:
      'bg-green-50 border border-green-200 rounded-md p-3 text-sm text-green-700 dark:bg-green-500/10 dark:border-green-500/30 dark:text-green-200',
        info:
      'bg-blue-50 border border-blue-200 rounded-md p-3 text-sm text-blue-700 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-200'
    } as const;
