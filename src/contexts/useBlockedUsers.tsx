...
export const BlockedUsersProvider: React.FC<React.PropsWithChildren<object>> = ({ children }) => {
    const { user } = useAuth();
    ...
    const block = React.useCallback(async (userId: number) => {
        if (!user) throw new Error('Not authenticated');
        setBlockedUsers((prev) => (prev ? (prev.includes(userId) ? prev : [...prev, userId]) : [userId]));
        try {
            ...
            try {
                queryClient.removeQueries({
                    predicate: (query) => {
                        ...
                        return ['posts', 'events', 'channels', 'channel', 'kudos-history', 'handshakes'].includes(first);
                    }
                });
            }
            catch (err) {
            ...
            setBlockedUsers((prev) => prev ? prev.filter((id) => id !== userId) : []);
            throw err;
        }
    }, [user]);

    const unblock = React.useCallback(async (userId: number) => {
        if (!user) throw new Error('Not authenticated');
        setBlockedUsers((prev) => (prev ? prev.filter((id) => id !== userId) : []));
        try {
            ...
            setBlockedUsers((prev) => (prev ? [...prev, userId] : [userId]));
            throw err;
        }
    }, [user]);

    const value = React.useMemo(() => ({ blockedUsers, loading, refresh, block, unblock }), [blockedUsers, loading, refresh, block, unblock]);
