...
export default function useUserLocation(opts: { enabled?: boolean } = {}): ReturnProps {
    ...
    useEffect(() => {
        ...
        const cacheApprox = (loc: MapCoordinates) => {
            try { sessionStorage.setItem(SESSION_KEY, JSON.stringify({ ...loc, ts: Date.now() })); }
            catch {
                // noop
            }
        };
        ...
        const fetchApproxLocation = async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    setErr('Location unavailable (network limit). Use the search box.');
                    return;
                }

                const data = await res.json();
                if (data?.latitude && data?.longitude) {
                    const approx = { latitude: data.latitude, longitude: data.longitude };
                    cacheApprox(approx);
        ...
        const tryGeolocation = () => {
            ...
            navigator.geolocation.getCurrentPosition(
                (pos) => {
                    ...
                    safeSet(setLocation, { latitude: pos.coords.latitude, longitude: pos.coords.longitude });
                    clearErr();
                },
                (err) => {
                    if (fallbackTimer) clearTimeout(fallbackTimer);
                    if (err?.code === 1) setErr('Location permission denied. Use the search box.');
                    else if (err?.code === 3) setErr('Location timed out. Use the search box.');
