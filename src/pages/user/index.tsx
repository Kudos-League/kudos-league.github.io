...
export default function UserProfile() {
      ...
      try {
        if (isLoggedInUser && userProfile) {
          ...
          if (!posts.length && authState?.token) {
            const [fetchedPosts, fetchedHandshakes, fetchedEvents] = await Promise.all([
  ...
  const handleUpdate = async (formData: Partial<UserDTO>) => {
    ...
    try {
      const updatedUser = await updateUser(formData, targetUserID.toString(), token);
      setUser(updatedUser);
    } catch (e) {
      console.error(e);
  ...
  if (!isLoggedIn) {
    return <div className="text-red-600 text-center mt-10">Please log in to view user details.</div>;
  }

  if (!user) {
    return <div className="text-red-600 text-center mt-10">Loading user details...</div>;
  }
