...
export default function CreatePost() {
  ...
  const handleTagsChange = useCallback(
    (tags: { id: string; name: string }[]) => {
      ...
      const hasBad = tagNames.some((t) => badwords.includes(t.toLowerCase().trim()));
  ...
  if (showLoginForm) return <Login onSuccess={() => setShowLoginForm(false)} />;
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex gap-3">
        ...
        <button onClick={() => setShowModal(true)} className="text-blue-600 underline ml-auto">
          ℹ️ Info
        </button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-lg max-w-md w-full">
            ...
            <button onClick={() => setShowModal(false)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded">
              Close
            </button>
          </div>
        </div>
      ...
      {postType === "gift" && <GiftType selected={giftType} onSelect={setGiftType} />}
      ...
      <select
        ...
        onChange={(e) => form.setValue("categoryID", parseInt(e.target.value))}
      >
      ...
      <Input name="files" label="Attach Images" form={form} type="file-image" multipleFiles />

      <TagInput initialTags={form.watch("tags")} onTagsChange={handleTagsChange} />
      {badWordFlag && <p className="text-red-600 text-sm">Tag contains a blocked word.</p>}
