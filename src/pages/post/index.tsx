...
import { ChannelDTO, CreateHandshakeDTO, Post as PostType  } from "shared/api/types";
...
const Post = () => {
  ...
  if (loading) return <div className="text-center mt-20 text-lg">Loading post...</div>;
  if (error) return (
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="flex items-center gap-4 mb-6">
        <AvatarComponent
          ...
          size={50}
          onClick={() => postDetails.sender?.id && navigate(`/user/${postDetails.sender.id}`)}
        />
        <div>
          <h2 className="font-bold text-lg">{postDetails.sender?.username || "Anonymous"}</h2>
          <p className="text-sm text-gray-500">Kudos: {postDetails.sender?.kudos ?? 0}</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        {postDetails.category?.name && (
          <p className="text-sm italic text-gray-600">Category: {postDetails.category.name}</p>
        )}
        <div className="flex gap-2 mt-2">
          <span className={`px-2 py-1 rounded text-white text-xs ${postDetails.type === "Request" ? "bg-blue-500" : "bg-green-500"}`}>
            {postDetails.type}
          </span>
          <span className="px-2 py-1 rounded bg-gray-700 text-white text-xs">{postDetails.status}</span>
        </div>
      </div>

      {/* Like, Dislike, Report */}
      <div className="flex gap-6 items-center my-4">
        <button onClick={() => likePost(parseInt(postDetails.id), true, token)} disabled={liked === true}>
          👍 <span className={liked === true ? "text-green-600" : "text-gray-400"}>Like</span>
        </button>
        <button onClick={() => likePost(parseInt(postDetails.id), false, token)} disabled={liked === false}>
          👎 <span className={liked === false ? "text-red-600" : "text-gray-400"}>Dislike</span>
        </button>
        <button onClick={() => alert("TODO: open report modal")}>⚠️ Report</button>
      </div>
      ...
      <div className="bg-white shadow p-4 rounded mb-6">
        ...
        {postDetails.handshakes?.length ? (
          <div className="space-y-4">
            {(showAllHandshakes ? postDetails.handshakes : postDetails.handshakes.slice(0, 2)).map((handshake, index) => {
              const isPostOwner = user?.id === Number(postDetails.sender?.id);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-semibold">{handshake.sender?.username || "Unnamed"}</p>
                    <p className="text-sm text-gray-600">Status: {handshake.status}</p>
                    <p className="text-sm text-gray-600">Kudos: {handshake.sender?.kudos ?? 0}</p>
                  </div>
                  {isAcceptable && (
                    <button
                      onClick={() => handleAcceptHandshake(index)}
            ...
            {postDetails.handshakes.length > 2 && !showAllHandshakes && (
              <button
        ...
        {user?.id !== Number(postDetails.sender?.id) &&
          !postDetails.handshakes?.some((h) => h.sender?.id === user?.id) && (
            <div className="mt-4 flex justify-center">
              ...
              >
                {creatingHandshake ? "Creating..." : "Start Handshake"}
              </button>
            </div>
          )}
      </div>
